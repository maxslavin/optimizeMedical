...
* 	@license      :	license.txt
* 	@filesource   :	wp-content/themes/health-center/search.php
*/
?>
<?php get_header(); ?>
...
<div class="container">
	<div class="row hc_blog_wrapper">		
		...
		<div class="col-md-8">
		<?php if ( have_posts() ) : ?>
				<h2 class="hc_search_head"><?php printf( __( "Search Results for: %s", 'health' ), '<span>' . get_search_query() . '</span>' ); ?></h2>
			...
			<div class="hc_blog_post_content"><p><?php the_excerpt(); ?></p></div>
			</div>
			<?php endwhile ?>
			...
			<?php else : ?>
						<h2><?php _e( "Nothing Found", 'health' ); ?></h2>
						<div class="">
						<p><?php _e( "Sorry, but nothing matched your search criteria. Please try again with some different keywords.", 'health' ); ?>
						</p>
						<?php get_search_form(); ?>
						</div>	
			<?php endif; ?>
