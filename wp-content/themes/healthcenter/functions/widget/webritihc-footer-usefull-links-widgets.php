<?php
add_action( 'widgets_init','webriti_hc_footer_widget'); 
   function webriti_hc_footer_widget() { return   register_widget( 'HC_footer_usefull_page_widget' ); }
/**
 * Adds HC_footer_usefull_page_widget widget.
 */
class HC_footer_usefull_page_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'HC_footer_usefull_page_widget', // Base ID
			__('Health Center Footer Usefull Links', 'health'), // Name
			array( 'description' => __( 'The most Usefull page link on your site', 'health' ), ) // Args
		);
	}
		...
		<?php	$loop = new WP_Query(array( 'post_type' => 'page', 'showposts' => $number_of_pages ));
				if( $loop->have_posts() ) : 
				  while ( $loop->have_posts() ) : $loop->the_post();?>				
					<a href="<?php the_permalink(); ?>"><i class="fa fa-arrow-circle-right"></i><?php the_title(); ?></a>
					<?php endwhile; ?>			
				<?php endif; ?>
		</div>
		<?php		
	...
	public function form( $instance ) {
		...
		<p>
		<label for="<?php echo $this->get_field_id( 'number_of_pages' ); ?>"><?php _e( 'Number of pages to show:','health' ); ?></label> 
